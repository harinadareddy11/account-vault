// src/navigation/TabNavigator.tsx - MODERN TAB BAR WITH CUSTOM ICONS
...
import CategoriesHomeScreen from '../screens/CategoriesHomeScreen';
...
const FolderIcon = ({ color, focused }: { color: string; focused: boolean }) => (
  <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
    <Path
...
const KeyIcon = ({ color, focused }: { color: string; focused: boolean }) => (
  <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
    <Circle
      cx="15.5"
      cy="8.5"
    ...
    />
    <Path
      d="M11.5 11.5L3 20M3 20L5 18M3 20L5 22"
      stroke={color}
      strokeWidth={focused ? "2.5" : "2"}
      strokeLinecap="round"
      strokeLinejoin="round"
...
export default function TabNavigator({ userId, masterPassword }: TabNavigatorProps) {
  return (
    <Tab.Navigator
      screenOptions={{
        headerShown: false,
    ...
    >
      ...
      <Tab.Screen
        ...
        options={{
          ...
          tabBarIcon: ({ color, focused }) => (
            <View style={[styles.iconContainer, focused && styles.iconContainerActive]}>
              <KeyIcon color={color} focused={focused} />
            </View>
          ),
        }}
      >
